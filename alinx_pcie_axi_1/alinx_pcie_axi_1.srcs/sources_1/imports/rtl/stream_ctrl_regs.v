...
module stream_ctrl_regs #(
    ...
    localparam [3:0] ST_WAIT_REP    = 4'd7;
    localparam [3:0] ST_CLEAR_START = 4'd8;
    localparam [3:0] ST_WRITE_STAT  = 4'd9;
    localparam [3:0] ST_WRITE_BEATS = 4'd10;
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            case (state)
                ...
                ST_WAIT_REP: begin
                    // REPEAT==0 => use LENGTH as total; else total = SEQ_LEN * REPEAT
                    if (bram_doutb != 32'd0) begin
                        length_bytes  <= pending_seq * bram_doutb;
                        repeat_count  <= bram_doutb;
                    end else begin
